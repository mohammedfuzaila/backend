...
Runs during build on Render to:
  ...
  3. Seed default data for Projects, Testimonials, Skills, Experience, Services, Blogs
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        from api.models import (
            AboutSection, HeroSection, SeoSettings, SiteSettings,
            Skill, Project, Experience, Service, Testimonial, BlogPost
        )
        ...
        hero, created = HeroSection.objects.get_or_create(id=1, defaults=hero_data)
        if created:
        ...
        about, created = AboutSection.objects.get_or_create(id=1, defaults=about_data)
        if created:
        ...
        self.stdout.write(self.style.SUCCESS('\nInitial data setup complete (including dummy fallback data)!'))
